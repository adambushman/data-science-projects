...
setwd('full-projects/R/nba-player-position-roles')
```
...
```{r}
cluster_hclust <- function(k, data, dist) {
    # Run the algorithm
    fit <- hclust(
        dist, 
        method = "ward.D2"
    )
    cluster_assignments <- cutree(fit, 3)

    wss <- calc_wss(cluster_assignments, data)
    avg_sil <- calc_avg_sil(cluster_assignments)
    return(list("wss" = wss, "avg_sil" = avg_sil))
}
```
